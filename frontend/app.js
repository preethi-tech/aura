...
function tierNameForIndex(v) {
  if (v == null) return "No data";
  if (v < 25) return "Tier 0 — Stable";
  if (v < 50) return "Tier 1 — Notice";
  if (v < 75) return "Tier 2 — Connect";
  return "Tier 3 — Reach out now";
}
          ...
          callbacks: {
            afterLabel: function(context) {
              if (context.datasetIndex === 0) {
                return tierNameForIndex(context.parsed.y);
              }
              return "";
            },
          },
        ...
        annotation: {
          annotations: {
            tier1: {
              ...
              label: {
                display: true,
                content: "Tier 1 (Notice)",
                position: "start",
                backgroundColor: c.tier0,
                color: "#fff",
            ...
            tier3: {
              ...
              label: {
                display: true,
                content: "Tier 3 (Reach out)",
                position: "start",
                backgroundColor: c.tier3,
                color: "#fff",
                font: { size: 11, weight: "bold" },
                padding: 4,
                yAdjust: -8,
...
function wireFirebaseAuthState() {
  ...
  firebase.auth().onIdTokenChanged(async (user) => {
    ...
    try { await refresh(); } catch (e) { console.warn("Auth refresh:", e); }
  });
}
...
function wireEvents() {
  ...
  document.getElementById("googleSignIn").addEventListener("click", async () => {
    ...
    try {
      ...
      // Show user email
      const emailEl = document.getElementById("userEmail");
      if (emailEl) emailEl.textContent = result.user.email || "Signed in";
      const menuEl = document.getElementById("userMenu");
      if (menuEl) menuEl.classList.remove("hidden");

      // Refresh data with authenticated user
      try { await refresh(); } catch (e) { console.warn("Post-login refresh:", e); }
    } catch (error) {
